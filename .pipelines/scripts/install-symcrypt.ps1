...
$global:install_method = $null
$global:nuget_exe_path = $null
$global:nuget_install_url = "https://dist.nuget.org/win-x86-commandline/latest/nuget.exe"
$global:nuget_install_dir = "$env:ProgramFiles\NuGet"
...
# Installs the NuGet command-line client to this machine. A path to the NuGet
# executable is returned.
function install_nuget
{
    # create the NuGet install directory, if it doesn't already exist
    New-Item -Path "$global:nuget_install_dir" -ItemType "directory" -Force | Out-Null

    # download nuget.exe
    $nuget_exe_path = Join-Path -Path "$global:nuget_install_dir" -ChildPath "nuget.exe"
    Write-Host "Downloading NuGet from: `"$global:nuget_install_url`" to: `"$nuget_exe_path`"."
    Invoke-WebRequest -Uri "$global:nuget_install_url" -OutFile "$nuget_exe_path" | Out-Null

    # make sure the file was downloaded
    if (!(Test-Path "$nuget_exe_path" -PathType "Leaf"))
    {
        throw "Failed to download NuGet executable to: `"$nuget_exe_path`""
    }
    Write-Host "Successfully downloaded NuGet executable to: `"$nuget_exe_path`"."

    return "$nuget_exe_path"
}

# This function uses `$global:nuget_exe_path` to add the NuGet executable to
# the shell's path, if it isn't already present.
function add_nuget_to_path
{
    # add the NuGet install directory to the system PATH
    $current_path = [System.Environment]::GetEnvironmentVariable("PATH", [System.EnvironmentVariableTarget]::User)

    # split the PATH into its components and see if the NuGet install directory
    if (!($current_path -Split ';' | Where-Object { $_ -eq $global:nuget_install_dir }))
    {
        $new_path = "$current_path;$global:nuget_install_dir"
        Write-Warning "NEW PATH: $new_path"
        [System.Environment]::SetEnvironmentVariable("PATH", "$new_path", [System.EnvironmentVariableTarget]::User)
        Write-Host "Added NuGet install directory (`"$global:nuget_install_dir`") to system PATH."
    }
}
...
function download_nuget_release
{
    ...
    while ($attempt -lt $retry_count)
    {
        try
        {
            ...
            $proc = Start-Process -FilePath "$global:nuget_exe_path" `
                                  -Wait `
                                  -PassThru `
                                  -NoNewWindow `
...
function get_dll_path
{
    ...
    # if we are installing using the GitHub method, we don't need to search for
    # a DLL that matches our architecture; we already ensured this when we
    # downloaded the asset from GitHub.
    if ($global:install_method -eq "github")
    {
        # if multiple DLLs were found, warn the user and select the first one
        if ($dll_paths.Count -gt 1)
        {
            $msg = "Found multiple SymCrypt DLLs in install path: `"$global:install_path`":`n"
            ...
            return "$($dll_path.FullName)"
        }

        # at this point, we should have exactly one DLL
        Write-Host "Found SymCrypt DLL: `"$($dll_paths[0].FullName)`"."
        return "$($dll_paths[0].FullName)"
    }
    # otherwise, if we're not installing via GitHub, iterate through the DLLs
    # and make sure the path they are in matches the architecture we're looking
    # for
    else
    {
        $dll_arch_matches = @()
...
function main
{
    ...
    if ($InstallMethod -eq $null -or $InstallMethod -eq "")
    {
        ...
        $global:install_method = "nuget"
    }
    else
    {
        $global:install_method = $InstallMethod.ToLower().Trim()
        if ($global:install_method -ne "github" -and $global:install_method -ne "nuget")
        {
    ...
    Write-Host "Using installation method: `"${global:install_method}`"."
    ...
    # the internal feed.
    if ($global:install_method -eq "nuget")
    {
        ...
        # if a NuGet install path was provided, save it
        if ($NugetInstallPath -ne $null -and $NugetInstallPath -ne "")
        {
            $global:nuget_install_dir = $NugetInstallPath
        }
        # otherwise, set the default to be within the user's local application
        # data (the path will look something like this):
        #   C:\Users\USERNAME\AppData\Local\NuGet
        else
        {
            $global:nuget_install_dir = [System.Environment]::GetFolderPath("LocalApplicationData")
            $global:nuget_install_dir = Join-Path -Path "${global:nuget_install_dir}" -ChildPath "NuGet"
        }

        # make sure NuGet is installed. If it's not, we need to install it now
        $nuget_exe = Get-Command "nuget.exe" -ErrorAction Silent
        if ($nuget_exe -eq $null)
        {
            $global:nuget_exe_path = install_nuget
        }
        else
        {
            $global:nuget_exe_path = get_full_path -Path $nuget_exe.Path
        }
        Write-Host "NuGet executable found at: `"$global:nuget_exe_path`"."

        # make sure NuGet is on the system PATH
        add_nuget_to_path
    }
    ...
    # install the DLL to Windows\System32, if we are running with admin
    # privileges
    ...
    # if we're not running in admin privileges, log a warning indicating that
    # the DLL was not installed to System32
    else
    {
        $msg = "This script was not executed with administrator privileges."
        $msg = "$msg As a result, the SymCrypt DLL was not installed to `"$dll_dest`"."
        $msg = "$msg This current shell instance has been configured to use SymCrypt via the $global:symcrypt_lib_path_name environment variable,"
        $msg = "$msg but other processes may not be able to find the SymCrypt DLL unless it is manually copied to `"$dll_dest`"."
        Write-Warning "$msg"
    }
