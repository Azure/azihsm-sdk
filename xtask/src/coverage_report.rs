...
impl Xtask for CoverageReport {
    fn run(self, ctx: XtaskCtx) -> anyhow::Result<()> {
        ...
        cmd!(
            sh,
            r"cargo llvm-cov report --cobertura --output-path ./target/reports/cobertura_sdk.xml --ignore-filename-regex xtask[/\\]"
        ).run()?;
        ...
        cmd!(
            sh,
            r"cargo llvm-cov report --json --summary-only --output-path ./target/reports/sdk-cov.json --ignore-filename-regex xtask[/\\]"
        ).run()?;

        // Generate HTML report
        log::info!("Generating HTML report");
        cmd!(sh, r"cargo llvm-cov report --html --output-dir ./target/reports/sdk-cov/ --ignore-filename-regex xtask[/\\]").run()?;
