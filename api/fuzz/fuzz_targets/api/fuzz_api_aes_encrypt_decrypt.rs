...
pub struct AesEncryptDecryptInput {
    pub key: HsmKeyHandle,
    pub mode: AesMode,
    pub data: Vec<u8>,
    pub iv: [u8; 16],
    pub key_type: AesInputKeyType,
    pub digest_kind: DigestKind,
    pub use_valid_keyhandle: bool,
}

#[cfg(target_os = "linux")]
fuzz_target!(|input: AesEncryptDecryptInput| {
    api_fuzz_test(
        ...
        &|device: &HsmDevice, _path: &str| {
            ...
            if input.use_valid_keyhandle {
                let key_type = aes_input_key_type_to_key_type(input.key_type);
            ...
            let _ = app_session.aes_encrypt_decrypt(
                &keyhandle,
                input.mode,
                input.data.clone(),
                input.iv,
