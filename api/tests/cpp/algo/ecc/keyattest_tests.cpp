...
// Parameterized test for different ECC curves
class azihsm_ecc_keyattest_curves : public azihsm_ecc_keyattest,
                                    public ::testing::WithParamInterface<azihsm_ecc_curve>
{
};

TEST_P(azihsm_ecc_keyattest_curves, attest_key)
{
    azihsm_ecc_curve curve = GetParam();

    part_list_.for_each_part([curve](std::vector<azihsm_char> &path) {
        ...
        AutoKey pub_key;
        auto err =
            generate_ecc_keypair(session.get(), curve, true, priv_key.get_ptr(), pub_key.get_ptr());
...
// Instantiate tests for all supported ECC curves
INSTANTIATE_TEST_SUITE_P(
    EccCurves,
    azihsm_ecc_keyattest_curves,
    ::testing::Values(AZIHSM_ECC_CURVE_P256, AZIHSM_ECC_CURVE_P384, AZIHSM_ECC_CURVE_P521)
);

TEST_F(azihsm_ecc_keyattest, attest_with_empty_report_data)
{
...
TEST_F(azihsm_ecc_keyattest, attest_invalid_key_handle)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        auto partition = PartitionHandle(path);
        auto session = SessionHandle(partition.get());
...
TEST_F(azihsm_ecc_keyattest, attest_public_key_fails)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        auto partition = PartitionHandle(path);
        auto session = SessionHandle(partition.get());
