...
TEST_F(azihsm_ecc_sign_verify, streaming_sign_verify_ecdsa_sha256_p256)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        ASSERT_EQ(sign_init_err, AZIHSM_STATUS_SUCCESS);
        ...
        ASSERT_EQ(update1_err, AZIHSM_STATUS_SUCCESS);
        ...
        ASSERT_EQ(update2_err, AZIHSM_STATUS_SUCCESS);
        ...
        ASSERT_EQ(update3_err, AZIHSM_STATUS_SUCCESS);

        // Finalize sign
        std::vector<uint8_t> signature(64);
        azihsm_buffer sig_buf{ signature.data(), static_cast<uint32_t>(signature.size()) };
        auto sign_final_err = azihsm_crypt_sign_final(sign_ctx, &sig_buf);
        ASSERT_EQ(sign_final_err, AZIHSM_STATUS_SUCCESS);
        ASSERT_EQ(sig_buf.len, 64);
        ...
        ASSERT_EQ(verify_init_err, AZIHSM_STATUS_SUCCESS);
...
TEST_F(azihsm_ecc_sign_verify, streaming_sign_verify_ecdsa_sha384_p384)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        ASSERT_EQ(azihsm_crypt_verify_final(verify_ctx, &sig_buf), AZIHSM_STATUS_SUCCESS);
    });
}
...
TEST_F(azihsm_ecc_sign_verify, streaming_verify_fails_with_wrong_data)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        azihsm_buffer wrong_msg_buf{
            const_cast<uint8_t *>(reinterpret_cast<const uint8_t *>(wrong_message)),
            static_cast<uint32_t>(strlen(wrong_message))
        };
        ASSERT_EQ(azihsm_crypt_verify_update(verify_ctx, &wrong_msg_buf), AZIHSM_STATUS_SUCCESS);
...
TEST_F(azihsm_ecc_sign_verify, streaming_sign_consistency_with_single_shot)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        ASSERT_EQ(
            azihsm_crypt_verify(&algo, pub_key, &data_buf, &single_sig_buf),
            AZIHSM_STATUS_SUCCESS
        );
        ASSERT_EQ(
            azihsm_crypt_verify(&algo, pub_key, &data_buf, &streaming_sig_buf),
            AZIHSM_STATUS_SUCCESS
        );
    });
}