...
TEST_F(azihsm_ecc_sign_verify, DISABLED_MANUAL_restore_key_and_verify)
{
    ...
    // Get API revision for init
    azihsm_api_rev api_rev = { .major = 0, .minor = 0 };
    err = get_test_api_rev(part_handle.get(), api_rev);
    if (err != AZIHSM_STATUS_SUCCESS)
    {
        azihsm_part_close(part_handle.get());
        ASSERT_EQ(err, AZIHSM_STATUS_SUCCESS) << "Failed to get API revision";
    }
