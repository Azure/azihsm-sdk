...
TEST_F(azihsm_aes_cbc, encrypt_decrypt_128)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        auto ciphertext =
            single_shot_crypt(key.get(), &crypt_algo, plaintext, sizeof(plaintext), true);
        ...
        auto decrypted =
            single_shot_crypt(key.get(), &crypt_algo, ciphertext.data(), ciphertext.size(), false);
...
TEST_F(azihsm_aes_cbc, encrypt_decrypt_128_with_padding)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        uint8_t plaintext[13] = { 0xDE, 0xAD, 0xBE, 0xEF, 0x01, 0x23, 0x45,
                                  0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x10 };
...
TEST_F(azihsm_aes_cbc, streaming_encrypt_decrypt_no_padding)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        // Encrypt using helper
        auto ciphertext =
            streaming_crypt(key.get(), &encrypt_algo, plaintext, plaintext_len, 16, true);
...
TEST_F(azihsm_aes_cbc, streaming_encrypt_decrypt_with_padding)
{
    part_list_.for_each_part([](std::vector<azihsm_char> &path) {
        ...
        uint8_t plaintext[27] = { 0xDE, 0xAD, 0xBE, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89,
                                  0xAB, 0xCD, 0xEF, 0x10, 0x20, 0x30, 0x40, 0x11, 0x22,
                                  0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB };
        size_t plaintext_len = sizeof(plaintext);

        // Encrypt using helper with 10-byte chunks
        auto ciphertext =
            streaming_crypt(key.get(), &encrypt_algo, plaintext, plaintext_len, 10, true);
