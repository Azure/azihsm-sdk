...
fn open_session_ex_co_authenticated() {
    ...
    let session = part
        .open_session_ex(rev, CO, SessionType::Authenticated)
...
fn open_session_ex_cu_plaintext() {
    ...
    let session = part
        .open_session_ex(rev, CU, SessionType::PlainText)
...
fn open_session_ex_rejects_unknown_psk_id() {
    ...
    let result = part.open_session_ex(rev, 2, SessionType::Authenticated);
