...
/// `open_session` fails when `GetSessionEncryptionKey` returns a
/// retryable error for `MAX_RETRIES + 1` consecutive calls, for every
/// retryable error code.
#[api_test]
fn test_open_session_fails_from_get_session_key_exhausted() {
    for error in OPEN_SESSION_RETRYABLE_ERRORS {
        let (part, creds, _ctx) = init_with_resiliency();
        let rev = part.api_rev_range().max();
        let before = op_call_count(DdiOp::GetSessionEncryptionKey);
        ...
        assert!(
            result.is_err(),
            "open_session should fail after exhausting all {MAX_RETRIES} retries with {error:?} on GetSessionEncryptionKey, got: {result:?}"
        );

        let expected = expected_op_calls(error, MAX_RETRIES + 1);
        assert_eq!(
            after - before,
            expected,
            "exhaustion on GetSessionEncryptionKey: expected {expected} calls for {error:?}, got {}",
            after - before,
        );
    }
}

/// `open_session` fails when `OpenSession` returns a retryable error
...
fn test_open_session_fails_from_open_session_exhausted() {
    for error in OPEN_SESSION_RETRYABLE_ERRORS {
        ...
        assert!(
            result.is_err(),
            "open_session should fail after exhausting all {MAX_RETRIES} retries with {error:?} on OpenSession, got: {result:?}"
        );
        ...
        assert_eq!(
            after - before,
            expected,
            "exhaustion on OpenSession: expected {expected} calls for {error:?}, got {}",
            after - before,
        );
    }
}
...
fn test_open_session_recovers_after_nssr() {
    ...
    // Trigger NSSR when the first DDI op of open_session is entered.
    ...
    assert!(
        result.is_ok(),
        "open_session should recover after NSSR via restore_partition, got: {result:?}"
    );

    // restore_partition called init_part which calls the BK3 op.
    assert!(
        bk3_after > bk3_before,
        "{op:?} should have been called during restore_partition after NSSR \
         (before: {bk3_before}, after: {bk3_after})"
    );
}

/// A first session opens and closes normally, then an NSSR during the
/// next `open_session` call invalidates device state. The retry path
/// restores the partition and opens successfully.
#[api_test]
fn test_open_session_recovers_after_nssr_between_sessions() {
    let (part, creds, _ctx) = init_with_resiliency();
    let rev = part.api_rev_range().max();

    let session1 = part
        .open_session(rev, &creds, None)
        .expect("First open_session should succeed before NSSR");

    // Close session1 before triggering NSSR — the simulator only
    // allows one session per file handle.
    drop(session1);

    // Trigger NSSR when the second open_session enters its first DDI op.
    inject_fault(FaultRule::reset_on_next(DdiOp::GetSessionEncryptionKey, 1));
    ...
    assert!(
        result.is_ok(),
        "Second open_session should recover after NSSR via restore_partition, got: {result:?}"
    );
}

/// Without resiliency enabled, `open_session` does not retry after an
/// NSSR — the `CredentialsNotEstablished` error propagates immediately.
#[api_test]
fn test_open_session_fails_after_nssr_without_resiliency() {
    let (part, creds) = init_without_resiliency();
    let rev = part.api_rev_range().max();
    ...
    assert!(
        result.is_err(),
        "open_session without resiliency should fail after NSSR, got: {result:?}"
    );
}

/// Two consecutive NSSRs are each followed by a successful
/// `open_session`. The retry-with-restore machinery handles both
/// events independently.
#[api_test]
fn test_open_session_recovers_after_consecutive_nssr() {
    let (part, creds, _ctx) = init_with_resiliency();
    let rev = part.api_rev_range().max();
    ...
    let session1 = part
        .open_session(rev, &creds, None)
        .expect("open_session should recover after first NSSR");
    drop(session1);
    ...
    let session2 = part
        .open_session(rev, &creds, None)
        .expect("open_session should recover after second NSSR");
    drop(session2);
}

/// After an NSSR, `restore_partition` calls `init_part` with the
...
/// (TPM path does not use the POTA endorsement callback).
#[api_test]
fn test_open_session_pota_reendorsement_after_nssr() {
    if use_tpm() {
        return;
    }
    let (part, creds, _ctx) = init_with_resiliency();
    ...
    assert!(
        result.is_ok(),
        "open_session should recover after NSSR + POTA re-endorsement, got: {result:?}"
    );

    assert!(
        cert_chain_after > cert_chain_before,
        "GetCertChainInfo should have been called by the POTA callback after NSSR \
         (before: {cert_chain_before}, after: {cert_chain_after})"
    );
}

// NSSR-triggered tests on OpenSession DDI op
//
// These mirror the GetSessionEncryptionKey NSSR tests above but
// trigger the reset at the OpenSession DDI command instead. This
// exercises a different failure point: GetSessionEncryptionKey
// succeeds, then NSSR fires when OpenSession is entered.

/// After an NSSR on the `OpenSession` DDI op, `open_session` detects
/// `CredentialsNotEstablished` and triggers `restore_partition` which
/// re-establishes credentials, then retries successfully.
#[api_test]
fn test_open_session_recovers_after_nssr_on_open_session() {
    let (part, creds, _ctx) = init_with_resiliency();
    let rev = part.api_rev_range().max();
    ...
    // Trigger NSSR when the OpenSession DDI op is entered.
    ...
    assert!(
        result.is_ok(),
        "open_session should recover after NSSR on OpenSession, got: {result:?}"
    );

    assert!(
        bk3_after > bk3_before,
        "{op:?} should have been called during restore_partition after NSSR on OpenSession \
         (before: {bk3_before}, after: {bk3_after})"
    );
}

/// A first session opens and closes normally, then an NSSR on the
/// `OpenSession` DDI op during the next `open_session` call
/// invalidates device state. The retry path restores and succeeds.
#[api_test]
fn test_open_session_recovers_after_nssr_on_open_session_between_sessions() {
    let (part, creds, _ctx) = init_with_resiliency();
    let rev = part.api_rev_range().max();
    ...
    // Trigger NSSR when the second open_session's OpenSession DDI op is entered.
    inject_fault(FaultRule::reset_on_next(DdiOp::OpenSession, 1));
    ...
    assert!(
        result.is_ok(),
        "Second open_session should recover after NSSR on OpenSession, got: {result:?}"
    );
}

/// Without resiliency enabled, `open_session` does not retry after an
/// NSSR on the `OpenSession` DDI op — the error propagates immediately.
#[api_test]
fn test_open_session_fails_after_nssr_on_open_session_without_resiliency() {
    let (part, creds) = init_without_resiliency();
    let rev = part.api_rev_range().max();
    ...
    assert!(
        result.is_err(),
        "open_session without resiliency should fail after NSSR on OpenSession, got: {result:?}"
    );
}

/// Two consecutive NSSRs on `OpenSession` are each followed by a
/// successful recovery.
#[api_test]
fn test_open_session_recovers_after_consecutive_nssr_on_open_session() {
    let (part, creds, _ctx) = init_with_resiliency();
    let rev = part.api_rev_range().max();

    // First NSSR on OpenSession → recover.
    inject_fault(FaultRule::reset_on_next(DdiOp::OpenSession, 1));
    let session1 = part
        .open_session(rev, &creds, None)
        .expect("open_session should recover after first NSSR on OpenSession");
    drop(session1);

    // Second NSSR on OpenSession → recover again.
    inject_fault(FaultRule::reset_on_next(DdiOp::OpenSession, 1));
    let session2 = part
        .open_session(rev, &creds, None)
        .expect("open_session should recover after second NSSR on OpenSession");
    drop(session2);
}

/// After an NSSR on `OpenSession`, `restore_partition` calls
...
/// (TPM path does not use the POTA endorsement callback).
#[api_test]
fn test_open_session_pota_reendorsement_after_nssr_on_open_session() {
    if use_tpm() {
        return;
    }
    let (part, creds, _ctx) = init_with_resiliency();
    ...
    assert!(
        result.is_ok(),
        "open_session should recover after NSSR on OpenSession + POTA re-endorsement, got: {result:?}"
    );

    assert!(
        cert_chain_after > cert_chain_before,
        "GetCertChainInfo should have been called by the POTA callback after NSSR on OpenSession \
         (before: {cert_chain_before}, after: {cert_chain_after})"
    );
}
