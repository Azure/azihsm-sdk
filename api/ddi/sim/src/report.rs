...
impl CoseSign1Object<'_> {
    ...
    pub fn encode(&self, out: &mut [u8]) -> Result<usize, MborError> {
...
impl KeyAttestationReport {
    /// CBOR encoding of the struct.
    pub fn encode(&self, out: &mut [u8]) -> Result<usize, MborError> {
        minicbor::encode(self, out).map_err(|_| MborError::EncodeError)?;
...
pub fn encode_rsa_public(n: &[u8], e: &[u8], out: &mut [u8]) -> Result<usize, MborError> {
    let out_len = out.len();
...
/// See Section 7, Key Objects in <https://www.rfc-editor.org/rfc/rfc9052>.
pub fn encode_ecc_public(crv: i8, x: &[u8], y: &[u8], out: &mut [u8]) -> Result<usize, MborError> {
    let out_len = out.len();
    let mut encoder = Encoder::new(out);
