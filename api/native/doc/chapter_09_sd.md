...
undersized or NULL output buffer is rejected with
...
probe (call once with a NULL/small buffer to learn the size, then retry
with an adequately sized buffer) is safe for this command.
