...
/// Forces all public API symbols to be retained by the linker.
///
/// This function is **not** part of the public API and must never be called
/// at runtime; it exists solely as a link-time anchor used by the
/// `azihsm_api_native_shim` crate.
///
/// Because this function lives in the crate root it can reach every private
/// module's symbols.  A dependent crate can call it (or take its address) to
/// ensure the linker includes all `#[no_mangle]` entry-points without
/// resorting to `extern "C"` declarations, which behave differently across
/// platforms (notably Windows/MSVC, where they create unresolved import-lib
/// symbol references rather than resolving against rlib object code).
#[doc(hidden)]
pub fn __azihsm_force_link() {
    core::hint::black_box([
        // crypto – digest
        crypto_digest::azihsm_crypt_digest as *const () as usize,
        crypto_digest::azihsm_crypt_digest_init as *const () as usize,
        crypto_digest::azihsm_crypt_digest_update as *const () as usize,
        crypto_digest::azihsm_crypt_digest_finish as *const () as usize,
        // crypto – encrypt / decrypt
        crypto_enc_dec::azihsm_crypt_encrypt as *const () as usize,
        crypto_enc_dec::azihsm_crypt_decrypt as *const () as usize,
        crypto_enc_dec::azihsm_crypt_encrypt_init as *const () as usize,
        crypto_enc_dec::azihsm_crypt_encrypt_update as *const () as usize,
        crypto_enc_dec::azihsm_crypt_encrypt_finish as *const () as usize,
        crypto_enc_dec::azihsm_crypt_decrypt_init as *const () as usize,
        crypto_enc_dec::azihsm_crypt_decrypt_update as *const () as usize,
        crypto_enc_dec::azihsm_crypt_decrypt_finish as *const () as usize,
        // crypto – sign / verify
        crypto_sign_verify::azihsm_crypt_sign as *const () as usize,
        crypto_sign_verify::azihsm_crypt_verify as *const () as usize,
        crypto_sign_verify::azihsm_crypt_sign_init as *const () as usize,
        crypto_sign_verify::azihsm_crypt_sign_update as *const () as usize,
        crypto_sign_verify::azihsm_crypt_sign_finish as *const () as usize,
        crypto_sign_verify::azihsm_crypt_verify_init as *const () as usize,
        crypto_sign_verify::azihsm_crypt_verify_update as *const () as usize,
        crypto_sign_verify::azihsm_crypt_verify_finish as *const () as usize,
        // handle management
        handle_table::azihsm_free_ctx_handle as *const () as usize,
        // key management
        key_mgmt::azihsm_key_gen as *const () as usize,
        key_mgmt::azihsm_key_gen_pair as *const () as usize,
        key_mgmt::azihsm_key_delete as *const () as usize,
        key_mgmt::azihsm_key_derive as *const () as usize,
        key_mgmt::azihsm_key_unwrap as *const () as usize,
        key_mgmt::azihsm_key_unwrap_pair as *const () as usize,
        key_mgmt::azihsm_key_unmask as *const () as usize,
        key_mgmt::azihsm_key_unmask_pair as *const () as usize,
        key_mgmt::azihsm_generate_key_report as *const () as usize,
        // key properties
        key_props::azihsm_key_get_prop as *const () as usize,
        // partition management
        partition::azihsm_part_get_list as *const () as usize,
        partition::azihsm_part_free_list as *const () as usize,
        partition::azihsm_part_get_count as *const () as usize,
        partition::azihsm_part_get_path as *const () as usize,
        partition::azihsm_part_open as *const () as usize,
        partition::azihsm_part_init as *const () as usize,
        partition::azihsm_part_close as *const () as usize,
        partition::azihsm_part_reset as *const () as usize,
        // partition properties
        partition_props::azihsm_part_get_prop as *const () as usize,
        // session management
        session::azihsm_sess_open as *const () as usize,
        session::azihsm_sess_close as *const () as usize,
        // session properties
        session_props::azihsm_session_get_prop as *const () as usize,
    ]);
}
