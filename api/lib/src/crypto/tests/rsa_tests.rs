...
mod tests {
    ...
    fn test_import_rsa_private_key_helper() {
        ...
        let mut priv_key_der_buffer = vec![0u8; 2048];
    ...
    fn test_rsa_ddi_wrap_unwrap_integration() {
        ...
        let aes_key_size = AesKeySize::Aes256;
        ...
        let wrap_len = crypto_pub_key
            .rsa_wrap_len(test_data.len(), aes_key_size, hash_algo)
            .expect("Failed to get wrap length");

        println!("Required wrap buffer size: {}", wrap_len);

        // Wrap the test data using azihsm_crypto
        let mut wrapped_buffer = vec![0u8; wrap_len];
        let wrapped_data = crypto_pub_key
            .rsa_wrap(
                &test_data,
                aes_key_size,
                hash_algo,
            ...
            .expect("Failed to wrap data using azihsm_crypto");

        println!("Wrapped data length: {}", wrapped_data.len());
        ...
        match unwrap_result {
            ...
            Err(e) => {
                ...
                match e {
                    mcr_ddi::DdiError::InvalidParameter
                    | mcr_ddi::DdiError::DeviceNotFound
                    | mcr_ddi::DdiError::DeviceNotReady => {
    ...
    #[test]
    #[ignore] // Temporarily disabled due to cbindgen parsing issues
    fn test_rsa_wrap_unwrap_multiple_sizes() {
        ...
        let test_cases = [
            (2048, AesKeySize::Aes128),
            (2048, AesKeySize::Aes256),
    ...
    fn test_rsa_aes_wrap_unwrap_round_trip() {
        ...
        let crypto_pub_key: RsaPublicKeyHandle = RsaPublicKeyHandle::rsa_key_from_der(&pub_key_der)
        ...
        let wrap_len = crypto_pub_key
            .rsa_wrap_len(aes_key_bytes.len(), aes_key_size, hash_algo)
            .expect("Failed to calculate RSA wrap length");

        // Wrap the AES key
        let mut wrapped_buffer = vec![0u8; wrap_len];
        let wrapped_aes_key = crypto_pub_key
            .rsa_wrap(
                &aes_key_bytes,
                aes_key_size,
                hash_algo,
