...
/// `DdiStatus::PendingKeyGeneration`, and `DdiStatus::KeyNotFound`
/// are surfaced as distinct `HsmError` variants to enable targeted
/// retry logic during partition initialization and key operations.
///
...
impl From<DdiError> for HsmError {
    fn from(err: DdiError) -> Self {
        match err {
            ...
            _ => {
                tracing::warn!(?err, "Unmapped DdiError → DdiCmdFailure");
                HsmError::DdiCmdFailure
            }
        }
    }
