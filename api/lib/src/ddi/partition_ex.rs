...
//! finished [`OpenSessionExResult`] alongside the partition handle.
//!
...
use super::session_ex::OpenSessionExResult;
use super::*;
...
/// * `session` - The finished CO session whose `session_id` /
///   `param_key` this request binds to.
...
pub(crate) fn init_part_ex(
    ...
    session: &OpenSessionExResult,
    mach_seed: &[u8],
    part_policy: &[u8],
    pota_thumbprint: &[u8],
) -> HsmResult<TborPartInitResp> {
    if part_policy.len() != PART_POLICY_LEN || pota_thumbprint.len() != POTA_THUMBPRINT_LEN {
        return Err(HsmError::InvalidArgument);
    }

    let mach_seed_envelope =
        seal_mach_seed_envelope(&session.param_key, session.session_id, mach_seed)?;
    ...
    let dev = inner.dev();
    let mut cookie = None;
    dev.exec_op_tbor(&req, &mut cookie).map_err(HsmError::from)
}
...
///
/// * `partition` - The HSM partition handle.
/// * `session` - The finished CO session this request binds to.
...
) -> HsmResult<TborFinalizePartResp> {
...
pub(crate) fn get_part_id_ex(
    partition: &HsmPartition,
    session: &OpenSessionExResult,
) -> HsmResult<TborGetPartIdResp> {
    let req = TborGetPartIdReq {
        session_id: session.session_id,
    };
