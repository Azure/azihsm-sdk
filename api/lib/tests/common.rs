...
pub(crate) fn wrap_data(wrapping_pub_key_der: Vec<u8>, data: &[u8]) -> Vec<u8> {
    ...
    let mut encrypted_aes_key = wrapping_pub_key
        .encrypt(
            aes_key,
            mcr_ddi_sim::crypto::rsa::RsaCryptoPadding::Oaep,
...
pub fn get_unwrapping_key(session: &HsmSession) -> HsmKeyHandle {
    ...
    loop {
        ...
        let resp = session.get_unwrapping_key();

        if let Err(err) = resp {
            assert!(matches!(err, HsmError::PendingKeyGeneration), "{:?}", err);
...
pub(crate) fn generate_rsa_der(key_type: KeyType) -> (Vec<u8>, Vec<u8>) {
    ...
    let (rsa_priv, rsa_pub) = generate_rsa(size).expect("Failed to generate RSA key pair");
    ...
    (
        ...
        rsa_pub
            .to_der()
            .expect("Failed to convert RSA Public Key to DER"),
    )
}
...
pub(crate) fn generate_ecc_der(key_type: KeyType) -> (Vec<u8>, Vec<u8>) {
    let curve_name = match key_type {
        ...
        KeyType::Ecc521Private | KeyType::Secret521 | KeyType::Ecc521Public => EccCurve::P521,
        _ => panic!("Unsupported ECC key type"),
    };

    let (ecc_priv, ecc_pub) = generate_ecc(curve_name).expect("Failed to generate ECC key");
    ...
    (
        ...
        ecc_pub
            .to_der()
            .expect("Failed to convert ECC Public Key to DER"),
    )
}
...
pub(crate) fn ecdh_derive(ecc_priv_der: &[u8], ecc_pub_der: &[u8]) -> Vec<u8> {
    ...
    let ecc_pub = EccPublicKey::from_der(ecc_pub_der, None)
        .expect("Failed to create ECC Public Key from DER");
...
pub(crate) fn hkdf_derive(
    ...
    let secret = mcr_ddi_sim::crypto::secret::SecretKey::from_bytes(data)
        .expect("Failed to create HKDF from bytes");

    let hash_algorithm = match digest_kind {
        DigestKind::Sha1 => mcr_ddi_sim::crypto::sha::HashAlgorithm::Sha1,
        DigestKind::Sha256 => mcr_ddi_sim::crypto::sha::HashAlgorithm::Sha256,
        DigestKind::Sha384 => mcr_ddi_sim::crypto::sha::HashAlgorithm::Sha384,
        DigestKind::Sha512 => mcr_ddi_sim::crypto::sha::HashAlgorithm::Sha512,
    };

    secret
        .hkdf_derive(hash_algorithm, salt, info, out_len)
...
pub(crate) fn hmac(key: &[u8], msg: &[u8]) -> Vec<u8> {
    let hash_algorithm = match key.len() {
        ...
        64 => mcr_ddi_sim::crypto::sha::HashAlgorithm::Sha512,
        _ => panic!("Unsupported key length for HMAC"),
    };

    let hmac_key =
        mcr_ddi_sim::crypto::hmac::HmacKey::from_bytes(key).expect("Failed to create HMAC key");

    hmac_key
        .hmac(msg, hash_algorithm)
...
pub(crate) fn generate_random_vector(size: usize) -> Vec<u8> {
    ...
    // Generate a vector of the specified size with random numbers
    let random_vector: Vec<u8> = (0..size).map(|_| rng.gen_range(0x0..=0xF)).collect();
