...
fn test_aes_xts_encrypt_decrypt() {
    api_test(common_setup, common_cleanup, |device, _path| {
        ...
        let aes_key = generate_aes(KeyType::AesXtsBulk256);
...
fn test_aes_xts_encrypt_data_size() {
    api_test(common_setup, common_cleanup, |device, _path| {
        ...
        //size 512 , allowed,  large than data size
        ...
        //size 8192 large than data size
        let data = generate_random_vector(256);
        let tweak: [u8; 16usize] = [0x4; 16usize];
        let input_len = 4096;
...
fn test_aes_xts_decrypt_data_size() {
    api_test(common_setup, common_cleanup, |device, _path| {
        ...
        assert!(decrypted_result.is_err(), "{:?}", decrypted_result);
