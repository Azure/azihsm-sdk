...
#[test]
fn test_aes_xts_tampered_tweak_value() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
...
#[test]
fn test_aes_xts_encrypt_decrypt_multi_times() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
...
#[test]
fn test_aes_xts_encrypt_data_size() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
        ...
        // size 256 but DUL is 512
        // -ve test case for DUL.
        ...
        // size 4096 allowed
        let data = generate_random_vector(4096);
        let tweak: [u8; 16usize] = [0x4; 16usize];
        let data_len = data.len();
        ...
        //size 4096 smaller than data size
        ...
        //size 256k smaller than data size
        ...
        //data unit length < buffer length
        let data = generate_random_vector(1024);
        let tweak: [u8; 16usize] = [0x4; 16usize];
        ...
        let result = app_session.aes_xts_encrypt_decrypt(
            ...
            &aes_key1_handle,
            &aes_key2_handle,
            512, // data unit length < buffer length
            tweak,
            data.clone(),
        );
        assert!(result.is_ok(), "result {:?}", result);
...
#[test]
fn test_aes_xts_decrypt_data_size() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
        ...
        // dul same as buffer for 256 there is no other option
        let decrypted_result = app_session.aes_xts_encrypt_decrypt(
            AesMode::Decrypt,
            &aes_key1_handle,
            &aes_key2_handle,
...
#[test]
fn test_aes_xts_encrypt_mismatched_datalen() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
...
#[test]
fn test_aes_xts_decrypt_mismatched_datalen() {
    api_test(common_setup, common_cleanup, |device, _path| {
        let app_session = common_open_app_session(device);
