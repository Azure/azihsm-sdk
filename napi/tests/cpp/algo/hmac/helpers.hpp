...
azihsm_error generate_ec_key_pair_for_derive(azihsm_handle session_handle, azihsm_handle &pub_key_handle,
                                             azihsm_handle &priv_key_handle, azihsm_ecc_curve curve)
{
    ...
    pub_props.push_back({.id = AZIHSM_KEY_PROP_ID_CLASS, .val = &pub_key_class, .len = sizeof(pub_key_class)});
    ...
    priv_props.push_back({.id = AZIHSM_KEY_PROP_ID_CLASS, .val = &priv_key_class, .len = sizeof(priv_key_class)});
    priv_props.push_back({.id = AZIHSM_KEY_PROP_ID_KIND, .val = &key_kind, .len = sizeof(key_kind)});
    priv_props.push_back({.id = AZIHSM_KEY_PROP_ID_EC_CURVE, .val = &curve, .len = sizeof(curve)});
    priv_props.push_back({.id = AZIHSM_KEY_PROP_ID_DERIVE, .val = &derive_prop, .len = sizeof(derive_prop)});

    azihsm_key_prop_list pub_prop_list = {.props = pub_props.data(), .count = static_cast<uint32_t>(pub_props.size())};
    azihsm_key_prop_list priv_prop_list = {.props = priv_props.data(),
                                           .count = static_cast<uint32_t>(priv_props.size())};

    return azihsm_key_gen_pair(session_handle, &ec_keygen_algo, &priv_prop_list, &pub_prop_list, &priv_key_handle,
                               &pub_key_handle);
}

azihsm_error derive_hmac_key_via_ecdh_hkdf(azihsm_handle session_handle, azihsm_handle server_priv_key,
                                           azihsm_handle client_pub_key, azihsm_key_kind hmac_key_kind,
                                           azihsm_handle &hmac_key_handle, azihsm_ecc_curve curve,
                                           azihsm_handle *base_secret_handle = nullptr)
{
    ...
    azihsm_key_prop pub_key_prop = {
        .id = AZIHSM_KEY_PROP_ID_PUB_KEY_INFO, .val = client_pub_key_data.data(), .len = client_pub_key_len};
    ...
    base_secret_props.push_back(
        {.id = AZIHSM_KEY_PROP_ID_CLASS, .val = &base_secret_class, .len = sizeof(base_secret_class)});
    base_secret_props.push_back(
    ...
    base_secret_props.push_back({.id = AZIHSM_KEY_PROP_ID_BIT_LEN, .val = &key_bits, .len = sizeof(key_bits)});

    azihsm_key_prop_list base_secret_prop_list = {.props = base_secret_props.data(),
                                                  .count = static_cast<uint32_t>(base_secret_props.size())};

    err = azihsm_key_derive(session_handle, &ecdh_algo, server_priv_key, &base_secret_prop_list,
    ...
    azihsm_algo_hkdf_params hkdf_params = {
        .hmac_algo_id = AZIHSM_ALGO_ID_HMAC_SHA256, .salt = &salt_buf, .info = &info_buf};
    ...
    hmac_key_props.push_back({.id = AZIHSM_KEY_PROP_ID_CLASS, .val = &hmac_key_class, .len = sizeof(hmac_key_class)});
    hmac_key_props.push_back({.id = AZIHSM_KEY_PROP_ID_KIND, .val = &hmac_kind, .len = sizeof(hmac_kind)});
    hmac_key_props.push_back({.id = AZIHSM_KEY_PROP_ID_BIT_LEN, .val = &hmac_key_bits, .len = sizeof(hmac_key_bits)});
    hmac_key_props.push_back({.id = AZIHSM_KEY_PROP_ID_SIGN, .val = &hmac_sign_prop, .len = sizeof(hmac_sign_prop)});
    ...
    err = azihsm_key_derive(session_handle, &hkdf_algo, temp_base_secret.get(), &hmac_key_prop_list, &hmac_key_handle);
...
inline azihsm_error generate_ecdh_keys_and_derive_hmac(azihsm_handle session_handle, azihsm_key_kind hmac_key_type,
                                                       EcdhKeyPairs &key_pairs, azihsm_handle &hmac_key_handle,
                                                       azihsm_ecc_curve curve)
{
    ...
    azihsm_error err =
        generate_ec_key_pair_for_derive(session_handle, key_pairs.server_pub_key, key_pairs.server_priv_key, curve);
    if (err != AZIHSM_ERROR_SUCCESS)
        return err;

    // Generate client EC key pair
    err = generate_ec_key_pair_for_derive(session_handle, key_pairs.client_pub_key, key_pairs.client_priv_key, curve);
    if (err != AZIHSM_ERROR_SUCCESS)
        return err;

    // Derive HMAC key
    err = derive_hmac_key_via_ecdh_hkdf(session_handle, key_pairs.server_priv_key, key_pairs.client_pub_key,
                                        hmac_key_type, hmac_key_handle, curve);
