...
TEST_F(azihsm_rsa_sign_verify, sign_verify_pkcs_sha256_with_imported_key)
{
    part_list_.for_each_part([this](std::vector<azihsm_char> &path) {
        ...
        auto import_err = import_keypair(wrapping_pub_key.get(), wrapping_priv_key.get(), rsa_private_key_der,
                                         import_props, imported_priv_key.get_ptr(), imported_pub_key.get_ptr());
        ...
        auto verify_err = azihsm_crypt_verify(&sign_algo, imported_pub_key.get(), &data_buf, &verify_sig_buf);
        ...
        auto verify_fail_err = azihsm_crypt_verify(&sign_algo, imported_pub_key.get(), &modified_buf, &verify_sig_buf);
