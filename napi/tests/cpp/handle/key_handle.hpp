...
struct KeyProps
{
    ...
    KeyProps()
        : key_class(0), key_kind(0), bits(0), is_session(true), can_encrypt(false), can_decrypt(false), can_sign(false),
          can_verify(false), can_wrap(false), can_unwrap(false), can_derive(false)
    {
    }
};
  ...
  public:
    KeyHandle(azihsm_handle sess_handle, const azihsm_algo *algo, const KeyProps &props)
        : handle_(0), key_class_(props.key_class), key_kind_(props.key_kind), bits_(props.bits),
          session_(props.is_session ? 1 : 0), encrypt_(props.can_encrypt ? 1 : 0), decrypt_(props.can_decrypt ? 1 : 0),
          sign_(props.can_sign ? 1 : 0), verify_(props.can_verify ? 1 : 0), wrap_(props.can_wrap ? 1 : 0),
    ...
    KeyHandle(KeyHandle &&other) noexcept
        : handle_(other.handle_), key_class_(other.key_class_), key_kind_(other.key_kind_), bits_(other.bits_),
          session_(other.session_), encrypt_(other.encrypt_), decrypt_(other.decrypt_), sign_(other.sign_),
          verify_(other.verify_), wrap_(other.wrap_), unwrap_(other.unwrap_), derive_(other.derive_)
    {
        other.handle_ = 0;
    }
