...
class PartitionListHandle
{
  public:
    ...
    uint32_t count() const
    {
        ...
        if (err != AZIHSM_ERROR_SUCCESS)
        {
            throw std::runtime_error("Failed to get partition count. Error: " + std::to_string(err));
        }
        return count;
    }
    ...
    std::vector<azihsm_char> get_path(uint32_t index) const
    {
        ...
        {
            throw std::runtime_error("Failed to get partition path size. Error: " + std::to_string(err));
        }
