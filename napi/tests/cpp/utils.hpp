...
inline azihsm_error generate_rsa_unwrapping_keypair(azihsm_handle session, azihsm_handle *priv_key_handle,
                                                    azihsm_handle *pub_key_handle)
{
    ...
    std::vector<azihsm_key_prop> priv_props_vec = std::vector<azihsm_key_prop>{
        {.id = AZIHSM_KEY_PROP_ID_BIT_LEN, .val = &props.key_size_bits, .len = sizeof(props.key_size_bits)},
        ...
        {.id = AZIHSM_KEY_PROP_ID_SESSION, .val = &props.session_key, .len = sizeof(props.session_key)},
    ...
    return azihsm_key_gen_pair(session, &keygen_algo, &priv_prop_list, &pub_prop_list, priv_key_handle, pub_key_handle);
}

/// Helper function to import a key pair (RSA or ECC) using RSA-AES key wrapping
inline azihsm_error import_keypair(azihsm_handle wrapping_pub_key, azihsm_handle wrapping_priv_key,
                                   const std::vector<uint8_t> &key_der, KeyProps props,
                                   azihsm_handle *imported_priv_key, azihsm_handle *imported_pub_key)
{
    ...
    return azihsm_keypair_unwrap(&unwrap_algo, wrapping_priv_key, &wrapped_key_buf, &priv_key_props, &pub_key_props,
                                 imported_priv_key, imported_pub_key);
}