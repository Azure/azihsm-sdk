...
fn bounded_salt(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Vec<u8>> {
    let len = usize::arbitrary(u)? % (HKDF_SALT_MAX_LEN + 1);
    Ok(u.bytes(len)?.to_vec())
}

fn bounded_info(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Vec<u8>> {
    let len = usize::arbitrary(u)? % (HKDF_INFO_MAX_LEN + 1);
    Ok(u.bytes(len)?.to_vec())
}
