...
static AZIHSM_RSA_KEY* azihsm_ossl_keymgmt_gen(AZIHSM_RSA_GEN_CTX* genctx, ossl_unused OSSL_CALLBACK* cb, ossl_unused void* cbarg)
{
...
static AZIHSM_RSA_GEN_CTX* azihsm_ossl_keymgmt_gen_init_common(void* ctx, int selection,
                                                             const OSSL_PARAM params[], int key_type)
{
...
static AZIHSM_RSA_GEN_CTX* azihsm_ossl_keymgmt_gen_init_rsa(void* ctx, int selection, const OSSL_PARAM params[])
{
    return azihsm_ossl_keymgmt_gen_init_common(ctx, selection, params, AIHSM_KEY_TYPE_RSA);
}

static AZIHSM_RSA_GEN_CTX* azihsm_ossl_keymgmt_gen_init_rsa_pss(void* ctx, int selection, const OSSL_PARAM params[])
{
...
static int azihsm_ossl_keymgmt_match(const AZIHSM_RSA_KEY* rsa_key1, const AZIHSM_RSA_KEY* rsa_key2, ossl_unused int selection)
{
    /*
...
static int azihsm_ossl_keymgmt_get_params(AZIHSM_RSA_KEY* key, OSSL_PARAM params[])
{
    ...
    if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_BITS)) != NULL && !OSSL_PARAM_set_uint32(p, key->genctx.pubkey_bits)) {
