...
CK_RV C_Initialize(CK_VOID_PTR pInitArgs)
{
     ...
     * initializing caller. */
    g_p11.os_locking_ok = os_locking_ok;

    memset(g_p11.slots, 0, sizeof(g_p11.slots));
    memset(g_p11.sessions, 0, sizeof(g_p11.sessions));
    g_p11.slot_count = 0;
    g_p11.next_session_handle = 1;

    CK_RV rv = p11_objstore_mem_create(&g_p11.store);
    if (rv != CKR_OK)
    {
        p11_unlock();
        return rv;
    ...
    if (n_slots < 0)
    {
        ...
        g_p11.store.ctx = NULL;
        p11_unlock();
        P11LOG("C_Initialize: slot enumeration failed");
        return CKR_FUNCTION_FAILED;
    }
    g_p11.slot_count = (CK_ULONG)n_slots;
    g_p11.initialized = true;
    p11_unlock();

    P11LOG("C_Initialize: OK (%d slot(s), os_locking=%d)", n_slots, (int)g_p11.os_locking_ok);
    return CKR_OK;
}
...
CK_RV C_Finalize(CK_VOID_PTR pReserved)
{
    ...
    {
        p11_hsm_logout(g_p11.slots[i].hsm_session);
        g_p11.slots[i].hsm_session = 0;
        g_p11.slots[i].user_logged_in = false;
        g_p11.slots[i].so_logged_in = false;
    }
    p11_hsm_close_all();
    if (g_p11.store.ctx != NULL)
    {
        g_p11.store.ops->teardown(g_p11.store.ctx);
        g_p11.store.ctx = NULL;
    }
    g_p11.initialized = false;
    p11_unlock();
    P11LOG("C_Finalize: OK");
    return CKR_OK;
}
...
CK_RV C_GetInfo(CK_INFO_PTR pInfo)
{
    ...
    pInfo->cryptokiVersion.major = AZIHSM_P11_CK_MAJOR;
    pInfo->cryptokiVersion.minor = AZIHSM_P11_CK_MINOR;
    p11_pad_str(pInfo->manufacturerID, sizeof(pInfo->manufacturerID), AZIHSM_P11_MANUFACTURER);
    pInfo->flags = 0;
    p11_pad_str(
        pInfo->libraryDescription,
        sizeof(pInfo->libraryDescription),
        AZIHSM_P11_LIBRARY_DESC
    );
    pInfo->libraryVersion.major = AZIHSM_P11_LIB_MAJOR;
    pInfo->libraryVersion.minor = AZIHSM_P11_LIB_MINOR;
    return CKR_OK;
}
