...
{
    CK_BYTE tmp[64];
    if (hex == NULL || bytes > sizeof(tmp) || strlen(hex) != bytes * 2)
    {
        return -1;
    }
...
void p11_config_load(p11_config *cfg)
{
    ...
    (void)hex_decode(getenv("AZIHSM_PKCS11_ID"), cfg->id, sizeof(cfg->id));
    (void)hex_decode(getenv("AZIHSM_PKCS11_PIN"), cfg->default_pin, sizeof(cfg->default_pin));
}
