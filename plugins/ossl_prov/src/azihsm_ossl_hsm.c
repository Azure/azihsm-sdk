...
/*
 * Read the header of the next DER element. On success *pp points at the
 * element content and *out_len holds the content length. Fails on malformed
 * input or a tag other than expected_tag.
 */
static OSSL_STATUS der_expect(const uint8_t **pp, long max_len, int expected_tag, long *out_len)
{
    int tag = 0, cls = 0;

    /* ASN1_get_object() sets 0x80 in the return value on parse errors */
    if ((ASN1_get_object(pp, out_len, &tag, &cls, max_len) & 0x80) || tag != expected_tag)
    {
        return OSSL_FAILURE;
    }
...
/*
 * Determine whether a PKCS#8-encoded RSAPrivateKey carries usable CRT
 * components. PKCS#1 makes prime1..coefficient mandatory fields, but keys
 * synthesized from (n, d) only (e.g. exported from another HSM or CNG)
 * encode them as zero-valued INTEGERs; such keys can only be imported in
 * plain RSA form.
 */
static bool pkcs8_rsa_has_crt_components(const uint8_t *der, long der_len)
{
    const uint8_t *p = der;
    ...
    if (der_expect(&p, end - p, V_ASN1_SEQUENCE, &len) != OSSL_SUCCESS)
    {
        return false;
    }

    /* version INTEGER (skip) */
    if (der_expect(&p, end - p, V_ASN1_INTEGER, &len) != OSSL_SUCCESS)
    {
        return false;
    }
    p += len;

    /* privateKeyAlgorithm AlgorithmIdentifier (skip) */
    if (der_expect(&p, end - p, V_ASN1_SEQUENCE, &len) != OSSL_SUCCESS)
    {
        return false;
    }
    ...
    /* privateKey OCTET STRING wrapping the RSAPrivateKey */
    if (der_expect(&p, end - p, V_ASN1_OCTET_STRING, &len) != OSSL_SUCCESS)
    {
        return false;
    }
    end = p + len;
    ...
    /* Skip version, modulus, publicExponent, privateExponent */
    for (i = 0; i < 4; i++)
    {
        if (der_expect(&p, end - p, V_ASN1_INTEGER, &len) != OSSL_SUCCESS)
        {
    ...
    /* prime1, prime2, exponent1, exponent2, coefficient must be non-zero */
    for (i = 0; i < 5; i++)
    {
        bool nonzero = false;
        long j;
...
azihsm_key_kind azihsm_rsa_detect_key_kind(const char *input_key_file)
{
    ...
    int pkcs8_len = 0;
    azihsm_key_kind kind = AZIHSM_KEY_KIND_RSA;

    if (input_key_file == NULL || input_key_file[0] == '\0')
    {
        return kind;
    }

    if (azihsm_file_load(input_key_file, &input_buf) != AZIHSM_STATUS_SUCCESS)
    {
        return kind;
    }

    if (input_buf.ptr != NULL && input_buf.len != 0 &&
        azihsm_ossl_normalize_der_to_pkcs8(
            input_buf.ptr,
            (long)input_buf.len,
            &pkcs8_buf,
            &pkcs8_len
        ) == OSSL_SUCCESS &&
        pkcs8_rsa_has_crt_components(pkcs8_buf, (long)pkcs8_len))
    {
