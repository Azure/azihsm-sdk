...
> **Important:** The default provider **must** be loaded alongside the azihsm provider. The provider will refuse to initialise if it cannot find the default provider in the process's default library context.
...
echo "/opt/openssl-3.0.3/lib" | sudo tee /etc/ld.so.conf.d/openssl-3.0.3.conf
sudo ldconfig
