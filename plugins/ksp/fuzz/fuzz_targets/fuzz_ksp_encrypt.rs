...
/// This fuzzing target creates a new key, then uses it to encrypt data.
// Windows imports
...
use crate::common::{
    ...
    azihsmksp_close,
    azihsmksp_encrypt,
    ENCRYPT_PLAINTEXT_BUFFER_MAXLEN,
    ENCRYPT_CIPHERTEXT_BUFFER_MAXLEN,
};
...
struct FuzzInput
{
    ...
    pub encrypt: payloads::NCryptEncryptDecryptPayload,
}
...
fuzz_target!(|input: FuzzInput| -> Corpus
{
    ...
    // is to test encryption, and as such, we want to create a valid key to
    // encrypt with.
    //
    ...
    input.encrypt.key.create.use_valid_algorithm = true;

    // request the creation of a key. If it fails, and we're running in
    // "careful" mode, return early
    let (result, key) = input.encrypt.key.create(azihsmksp);
    ...
    let result = input.encrypt.key.finalize(
    ...
    // create an input buffer to store the plaintext we want to encrypt (if
    // `input_buffer_is_enabled` is true)
    let mut inbuff;
    let mut inbuff_param: Option<&[u8]> = None;
    if input.encrypt.input_buffer_is_enabled
    {
        inbuff = input.encrypt.input_buffer.create(ENCRYPT_PLAINTEXT_BUFFER_MAXLEN);
        inbuff_param = Some(inbuff.as_mut_slice());
    }

    // create an output buffer to store the resulting ciphertext, in the same
    // way as the plaintext buffer
    let mut outbuff;
    let mut outbuff_param: Option<&mut [u8]> = None;
    if input.encrypt.output_buffer_is_enabled
    {
        outbuff = input.encrypt.output_buffer.create(ENCRYPT_CIPHERTEXT_BUFFER_MAXLEN);
        outbuff_param = Some(outbuff.as_mut_slice());
    }

    // invoke the encryption function. Check the result, if we're in "careful"
    // mode
    let (result, _value) = azihsmksp_encrypt(
        #[cfg(feature = "direct-invoke-dll")] azihsmksp,
        key,
        &input.encrypt,
        inbuff_param,
    ...
    let _ = input.encrypt.key.delete(
        #[cfg(feature = "direct-invoke-dll")] azihsmksp,
        key,
    );
    azihsmksp_close(azihsmksp);
