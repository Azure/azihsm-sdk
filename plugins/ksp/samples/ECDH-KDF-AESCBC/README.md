...
3. Use KBKDF or HKDF to derive the same AES key (using the shared secret) for both parties.
    * KBKDF ("Key Based Key Distribution Function") refers to the **SP800-108 HMAC in counter mode** KDF, as seen [here](https://learn.microsoft.com/en-us/windows/win32/api/bcrypt/nf-bcrypt-bcryptkeyderivation) in the NCrypt API documentation.
    * HKDF ("HMAC-based Key Distribution Function") is defined in [IETF RFC 5869](https://datatracker.ietf.org/doc/html/rfc5869), and is referred to in NCrypt by the `BCRYPT_HKDF_ALGORITHM` string.
...
### Prerequisite - AziHSM KSP DLL

Before you can run this sample, you'll need to ensure you have the AziHSM KSP (Key Storage Provider) DLL installed onto your system.
Look for this file: `C:\Windows\System32\azihsmksp.dll`.
If the file does not exist, please see [this README](../../README.md) to build and install the DLL yourself.

You will also need [Microsoft Cryptographic Provider Development Kit](https://learn.microsoft.com/en-us/windows/win32/seccng/cng-portal) installed.
