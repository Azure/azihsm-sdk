...
impl AesType {
    ...
    pub fn hsm_key_size(&self) -> AesKeySize {
        match self {
            ...
            AesType::Aes256Gcm | AesType::Aes256Xts => AesKeySize::AesBulk256,
        }
    }
    ...
    pub fn hsm_key_type(&self) -> KeyType {
        match self {
            ...
            AesType::Aes256Gcm | AesType::Aes256Xts => KeyType::AesBulk256,
        }
    }

    pub fn hsm_key_class(&self) -> KeyClass {
        match self {
            AesType::Aes256Gcm | AesType::Aes256Xts => KeyClass::AesBulk,
            _ => KeyClass::Aes,
        }
    }
...
pub fn init_ciphers(engine: &Engine) -> OpenSSLResult<()> {
    ...
    AZIHSM_CIPHER_NIDS.get_or_init(|| {
        vec![
            ...
            AesType::Aes256Gcm.nid(),
            AesType::Aes256Xts.nid(),
        ]
    });
    ...
    fn test_engine_ciphers_get_nids() {
        ...
        match result.unwrap() {
            EngineCiphersResult::Nids((nids, num_nids)) => {
                assert_eq!(num_nids, 5);
                assert_eq!(nids[0], AesType::Aes128Cbc.nid());
                assert_eq!(nids[1], AesType::Aes192Cbc.nid());
                assert_eq!(nids[2], AesType::Aes256Cbc.nid());
                assert_eq!(nids[3], AesType::Aes256Gcm.nid());
                assert_eq!(nids[4], AesType::Aes256Xts.nid());
            }
