...
impl KeyAttester {
    ...
    ///
    /// # Errors
    /// * `AttestationError::CborEncodeError` - If CBOR encoding fails during creation.
    ...
    ///
    /// # Errors
    /// * `AttestationError::InvalidArgument` - If key is not a P-384 ECC key.
    /// * `AttestationError::CborEncodeError` - If CBOR encoding fails during creation.
    pub fn sign(
        &mut self,
        key: &EccPrivateKey,
    ) -> Result<([u8; TAGGED_COSE_SIGN1_OBJECT_MAX_SIZE], usize), AttestationError> {
    ...
    pub fn parse(report: &[u8]) -> Result<Self, AttestationError> {
    ...
    pub fn verify(&self, key: &EccPublicKey) -> Result<(), AttestationError> {
        let payload = &self.report[..self.report_size];
        // Create the to-be-signed data blob.
        let (buffer, len) = CoseSign1::create_tbs(&self.protected_header, payload)?;
        ...
        let curve = key.curve()?;
        if curve != CryptoEccCurve::P384 {
            Err(AttestationError::InvalidArgument)?
        }
...
impl CoseKey {
    ...
    ///
    /// # Errors
    /// * `AttestationError::CborEncodeError` - If CBOR encoding fails.
    pub fn encode(&self) -> Result<([u8; PUBLIC_KEY_MAX_SIZE], u16), AttestationError> {
...
impl TryFrom<&EccPrivateKey> for CoseKey {
    ...
    ///
    /// # Errors
    /// * `AttestationError::InvalidArgument` - If the key is not a supported ECC key.
...
impl TryFrom<&RsaPrivateKey> for CoseKey {
    ...
    ///
    /// # Errors
    /// * `AttestationError::InvalidArgument` - If the key is not a supported RSA key.
...
impl CoseSign1 {
    ...
    fn create_payload(
        ...
        report_data: &[u8; REPORT_DATA_SIZE],
        vm_launch_id: &[u8; VM_LAUNCH_ID_SIZE],
    ) -> Result<([u8; PAYLOAD_MAX_SIZE], usize), AttestationError> {
    ...
    fn create_tbs(
        body_protected: &[u8],
        payload: &[u8],
    ) -> Result<([u8; SIG_STRUCTURE_MAX_SIZE], usize), AttestationError> {
    ...
    /// * `AttestationError::InvalidArgument` - If key is not a P-384 ECC key.
    /// * `AttestationError::CborEncodeError` - If CBOR encoding fails during creation.
    /// * `AttestationError::CoseSign1UnexpectedSignature` - If size of the signature is unexpected.
    fn sign(key: &EccPrivateKey, tbs: &[u8]) -> Result<[u8; SIGNATURE_SIZE], AttestationError> {
        // Ensure the key uses P-384.
        let curve = key.curve()?;
        if curve != CryptoEccCurve::P384 {
        ...
        if signature.len() != SIGNATURE_SIZE {
            ...
            Err(AttestationError::CoseSign1UnexpectedSignature)?
        }
...
mod tests {
    ...
    fn test_cose_sign1() {
        ...
        let result = generate_ecc(CryptoEccCurve::P384);
        assert!(result.is_ok());
        ...
        let result = generate_ecc(CryptoEccCurve::P256);
        assert!(result.is_ok());
        let (ecc_private, _) = result.unwrap();

        let result = CoseSign1::sign(&ecc_private, &buffer[..len]);
        assert_eq!(result, Err(AttestationError::InvalidArgument));
