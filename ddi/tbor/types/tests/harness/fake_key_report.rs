...
//! carries `pk_r` in its inner EC2 `COSE_Key` and binds the supplied
//! policy digest. The rest of the payload is zero-filled and the outer
//! `COSE_Sign1` signature is 96 zero bytes.
//!
...
/// `public_key_size`, and `policy_digest` are populated by this fake;
/// every other field is zeroed. Kept private to this module -- the sim
/// crate does not export a v2 payload struct, so the schema is duplicated
/// here.
...
/// whose inner EC2 `COSE_Key` carries `pk_r` and whose policy binding is
/// `policy_digest`.
///
...
pub(crate) fn fake_key_report_bytes(
    pk_r_sec1: &[u8; SEC1_PUB_LEN],
    policy_digest: &[u8; POLICY_DIGEST_SIZE],
) -> Vec<u8> {
    assert_eq!(
        pk_r_sec1[0], 0x04,
    ...
    fn fake_report_wraps_pk_r() {
        ...
        assert!(
            bytes
                ...
                .any(|w| w == [0xA5; POLICY_DIGEST_SIZE]),
            "emitted report must contain the supplied policy digest",
        );
    }
}
