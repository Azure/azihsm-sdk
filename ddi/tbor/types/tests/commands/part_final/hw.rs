...
//! Hardware end-to-end tests for the TBOR `PartFinal` command.
//!
//! These tests exercise the full native-driver path on real silicon:
//! `OpenSession -> PskChange -> PartInit -> PartFinal`. Every case uses a
//! syntactically valid POTA-to-PTA certificate chain so the assertions remain
//! valid for both the M1.0 surrogate-chain implementation and the later full
...
#[test]
fn part_final_full_flow_valid_chain_hw() {
    hw_test_reset(|dev| {
...
#[test]
fn part_final_accepts_prior_backup_across_nssr_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
        let seed = mach_seed();
...
#[test]
fn part_final_rejects_tampered_backup_and_allows_retry_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
        let seed = mach_seed();
...
#[test]
fn part_final_rejects_policy_mismatch_and_allows_retry_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
        let session = bootstrap_rotated_co(dev, &ROTATED_CO_PSK);
        let pta_pub = run_part_init(dev, &session, &fixture, &mach_seed());
...
#[test]
fn part_final_rejects_backup_from_different_mach_seed_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
...
#[test]
fn part_final_rejects_before_part_init_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
        let before = read_part_info(dev);
...
#[test]
fn part_final_rejects_cu_and_allows_co_retry_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
...
#[test]
fn part_final_rejects_second_finalize_hw() {
    hw_test_reset(|dev| {
        let fixture = PotaFixture::generate();
        let session = bootstrap_rotated_co(dev, &ROTATED_CO_PSK);
        let pta_pub = run_part_init(dev, &session, &fixture, &mach_seed());
