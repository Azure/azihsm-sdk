...
/// @file rsa_session_sign_verify_tests.cpp
///
...
class RSA_SessionKeyRoundTrip : public ::testing::Test
{
  protected:
    ProviderCtx prov_;
};
...
TEST_F(RSA_SessionKeyRoundTrip, sign_verify_pkcs1)
{
    ...
    // Negative — tampered data must NOT verify ----------------------------
...
TEST_F(RSA_SessionKeyRoundTrip, sign_verify_pss)
{
    auto pkey = generate_rsa_session_key(prov_.libctx());
    ASSERT_NE(pkey, nullptr) << "RSA session key generation failed";
...
/// key must fail.
TEST_F(RSA_SessionKeyRoundTrip, verify_fails_with_wrong_key)
{
