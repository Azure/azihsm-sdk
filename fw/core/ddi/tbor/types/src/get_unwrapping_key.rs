...
//! RSA-AES key-wrap a payload for [`UnwrapKey`](crate::unwrap_key).  The
...
///
/// `pub_key` is `#[tbor(mutable)]` so the handler can build the response
/// with the slot **reserved** (via `pub_key_reserve`) and have the PAL
/// write the wire public key straight into it (`decode_mut`) — no scratch
/// buffer and no copy.
#[tbor(response)]
