...
pub(crate) fn parse_open_enum(input: &syn::DeriveInput) -> syn::Result<DdiOpenEnum> {
    ...
    if !enum_attr.enumeration {
        ...
        return Err(syn::Error::new(
            syn::spanned::Spanned::span(&enum_attr.ident),
            msg,
        ));
    }
    ...
    match enum_attr.data {
        ...
        ast::Data::Struct(fields) => {
            ...
            let field = fields.fields.into_iter().next().ok_or_else(|| {
                let msg = format!("Failed to unwrap field for open_enum {}.", enum_attr.ident);
                syn::Error::new(syn::spanned::Spanned::span(&enum_attr.ident), msg)
            })?;
            let field_segments = match field.ty {
                syn::Type::Path(p) => p.path.segments,
                _ => {
                    let msg = format!(
                        "#[ddi(enumeration)] field is not a path type for {}.",
                        enum_attr.ident
                    );
                    return Err(syn::Error::new(
                        syn::spanned::Spanned::span(&enum_attr.ident),
            ...
            if field_segments.len() != 1 {
                let msg = format!(
                    "#[ddi(enumeration)] field type has unexpected segments for {}.",
                    enum_attr.ident
                );
                return Err(syn::Error::new(
                    syn::spanned::Spanned::span(&enum_attr.ident),
            ...
            let ident = field_segments
                .first()
                .ok_or_else(|| {
                    let msg = format!(
                        "Failed to unwrap field type segment for {}.",
                        enum_attr.ident
                    );
                    syn::Error::new(syn::spanned::Spanned::span(&enum_attr.ident), msg)
                })?
