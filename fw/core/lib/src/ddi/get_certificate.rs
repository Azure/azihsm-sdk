...
pub(crate) async fn get_certificate<'a>(
    hdr: &DdiReqHdr,
    ...
    let body: DdiGetCertificateReq = decoder.decode_data().map_err(|_| DDI_DECODE_FAILURE)?;
    ...
    let frame = DdiGetCertificateResp::frame(&mut encoder, len).map_err(|_| DDI_ENCODE_FAILURE)?;
