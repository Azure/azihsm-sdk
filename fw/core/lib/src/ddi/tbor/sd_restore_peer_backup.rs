...
//! TBOR `SdRestorePeerBackup` handler.
//!
...
//! the sender peer's attested key — recovers `SDMK` from
//! `prev_sd_mk_backup`, and returns the device-local backups so the
//! security domain can later be restored locally without the peer.
//!
//! It is [`SdRestoreRemoteBackup`](super::sd_restore_remote_backup) plus a
//! **peer-cloning policy gate**: the recovered backup originates from a
//! peer partition endorsed by the same POTA rather than a remote sealing
//! authority, so it is only accepted when the security domain's policy sets
//! `allow_peer_cloning`.  The provisioning back-end is shared with the
//! local and remote restores via
...
//!    verify the **sender** peer evidence against it: its cert chains are
//!    validated and anchored to the policy SATA key, its report's v2
//!    `policy_hash` must equal `SHA-384(policy)`, and its attested COSE_Key
//!    is recovered as `SndrPub`.
//! 3. Unmask `masked_sealing_key` → `RcvrPriv` (must be an
//!    [`SdSealing`](HsmVaultKeyKind::SdSealing) key) and derive `RcvrPub`.
//! 4. HPKE-Auth-open `pok_peer_backup` (`sk_r = RcvrPriv`, sender-auth
//!    `SndrPub`) → **BKS3**.
...
use azihsm_fw_ddi_tbor_types::TborSdRestorePeerBackupReq;
use azihsm_fw_ddi_tbor_types::TborSdRestorePeerBackupResp;
...
/// Length of the BKS3 carried inside the peer backup.
...
// A peer backup is `enc(97) ‖ ct(BKS3 48 + GCM tag 16 = 64)` = 161 B.
const _: () = assert!(SD_ENC_LEN + (BKS3_LEN + 16) == POK_REMOTE_BACKUP_LEN);

/// Handle a TBOR `SdRestorePeerBackup` request.
///
...
pub(crate) async fn handle<'p, P: HsmPal>(
    ...
    let mk_key_id = {
        let req = TborSdRestorePeerBackupReq::decode(&*req_buf)?;
    ...
    pal.alloc_scoped_async(io, async |alloc| -> HsmResult<()> {
        ...
        // ── Phase 1: policy binding + peer-cloning gate + sender evidence ──
        {
            let req = TborSdRestorePeerBackupReq::decode(&*req_buf)?;
            ...
            // The sender peer's report must attest to the same policy.
        ...
        // ── Phase 2: recover RcvrPriv, HPKE-open the peer backup, and
        // re-provision the SD.  The masked key, peer backup, and previous
        ...
        let peer_backup = alloc.dma_alloc(POK_REMOTE_BACKUP_LEN)?;
        let prev_sd_mk = alloc.dma_alloc(SD_MK_BACKUP_LEN)?;
        {
            let req = TborSdRestorePeerBackupReq::decode(&*req_buf)?;
            blob.copy_from_slice(req.masked_sealing_key());
            peer_backup.copy_from_slice(req.pok_peer_backup());
        ...
        let crypto_res = async {
            ...
            let inner = async {
                // ── Open the peer backup with RcvrPriv, authenticated by
                // the sender peer key (`SndrPub`).
                let (enc, ct) = peer_backup.split_at(SD_ENC_LEN);
                ...
                // to the vault (shared with the local / remote restore).
...
/// Encode the `SdRestorePeerBackup` response around the local backups.
fn encode_response<'p, P: HsmPal>(
    ...
    let resp = pal.dma_alloc_var(io, |buf| {
        let frame = TborSdRestorePeerBackupResp::encode(buf, 0, false)?
