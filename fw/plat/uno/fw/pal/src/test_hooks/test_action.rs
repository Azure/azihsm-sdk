...
//! - `TriggerCrash` (8) crashes this core. Its parameters arrive in the
//!   request's **opaque payload** — the `{1: action, 2: payload}` body
//!   carries `crash_info` not as a nested map but as a byte string whose
//!   bytes are the MBOR encoding of [`DdiTestActionCrashReqInfo`]. The
//!   handler decodes the payload, then decodes the crash info out of it.
//!
...
//! # Opaque payload
//!
//! An action that carries parameters puts them in one opaque field —
//! `{2: payload}`, a byte string holding the MBOR encoding of that
//! action's own request-info body. The `TestAction` request map itself is
//! therefore fixed at `{1: action, 2: payload?}` regardless of the action,
//! so adding an action never changes this opcode's wire schema. The trade
//! is that the payload bytes bypass the outer map's field validation, so a
//! handler that claims an action re-decodes and validates the body itself.
//! This diverges from `mcr-hsm`, which still carries each action's fields
//! as typed map entries; the two firmwares are no longer wire-compatible
//! for `TestAction`, by design (see the crate-level note in [`super`]).
...
pub(super) fn dispatch<'p>(
    ...
    match action {
        ...
        ACTION_TRIGGER_CRASH => {
            // `{1: action, 2: payload}`, where `payload` is the opaque byte
            // string carrying the MBOR-encoded crash info.
            if body_count.0 != 2 {
                return Err(HsmError::DdiDecodeFailed);
            }
            ...
            // Peel the opaque payload off the outer map (zero-copy). The
            // outer buffer must end right after it — the payload is the
            // last field.
            ...
            // Decode the action-specific body out of the opaque payload,
            // and reject any trailing bytes: the payload holds exactly one
            // MBOR-encoded `DdiTestActionCrashReqInfo` and nothing else.
            let payload_len = payload.len();
            let mut inner = MborDecoder::new(payload);
            let crash_info = DdiTestActionCrashReqInfo::mbor_decode(&mut inner)
                .map_err(|_| HsmError::DdiDecodeFailed)?;
            if inner.position() != payload_len {
                return Err(HsmError::DdiDecodeFailed);
            }
