...
impl CryptEnv for CryptoTestEnv {
    fn hmac384_tag(&self, key: &[u8], data: &[u8]) -> Result<[u8; 48], CryptoError> {
        hmac_sha_384(key, data).map_err(|_| CryptoError::HmacError)
    }
    ...
    fn aescbc256_encrypt(
        ...
        crypto::rand::rand_bytes(iv_in).map_err(|_| CryptoError::RngError)?;
