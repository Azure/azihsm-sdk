...
use masked_key::DecodedMaskedKey;
...
impl LMKeyDerive {
    ...
    /// * `Err(CryptoError)` - If there is an error during the generation process.
    pub fn bk_partition_gen<Env: CryptEnv>(
            ...
            || bk_partition_out.len() < BK_AES_CBC_256_HMAC384_SIZE_BYTES
        {
            *bk_partition_len = BK_AES_CBC_256_HMAC384_SIZE_BYTES;
            Err(CryptoError::OutputBufferTooSmall)?;
        }
    ...
    /// * `Err(CryptoError)` - If there is an error during the encoding process.
    ...
    pub fn encode_masked_key_metadata(
        ...
        let metadata = DdiMaskedKeyMetadata {
            ...
            key_label: MborByteArray::<128>::from_slice(key_label)
                .map_err(|_| CryptoError::MborEncodeFailed)?,
        };
        ...
        metadata
            ...
            .map_err(|_| CryptoError::MetadataEncodeFailed)?;

        if encoder.position() != len {
            Err(CryptoError::MetadataEncodeFailed)?;
        }
    ...
    /// * `Err(CryptoError)` - If there is an error during the decoding process.
    pub fn decode_masked_key_metadata(
        device_kind: DdiDeviceKind,
        encoded_metadata: &[u8],
    ) -> Result<DdiMaskedKeyMetadata, CryptoError> {
        let (pre_encode, _) = match device_kind {
            DdiDeviceKind::Physical => (true, true),
            _ => (false, false),
        };
        ...
        let metadata = DdiMaskedKeyMetadata::mbor_decode(&mut decoder)
            .map_err(|_| CryptoError::MetadataDecodeFailed)?;
    ...
    /// * `Err(CryptoError)` - If there is an error during the restoration process.
    ///
    /// # Note
    ...
    pub fn bmk_restore<'a, Env: CryptEnv>(
        ...
        bk: &[u8],
        bmk: &'a [u8],
    ) -> Result<DecodedMaskedKey<'a>, CryptoError> {
        MaskedKey::decode(env, bk, bmk, true).map_err(|_| CryptoError::MaskedKeyDecodeFailed)
    }
...
mod tests {
    ...
    struct TestCryptoEnv {
        ...
        ciphertext: Vec<u8>,
        hmac384_tag: [u8; 48],
        error: Option<(CryptoFunc, CryptoError)>,
    }
    ...
    impl CryptEnv for TestCryptoEnv {
        fn hmac384_tag(&self, _key: &[u8], _data: &[u8]) -> Result<[u8; 48], CryptoError> {
        ...
        fn kbkdf_sha384(
            ...
            while written < out_len {
                ...
                let hash = sha(HashAlgorithm::Sha384, &block).unwrap();
    ...
    fn test_bk_partition_gen_insufficient_buffer() {
        ...
        assert!(matches!(result, Err(CryptoError::OutputBufferTooSmall)));
        assert_eq!(bk_len, BK_AES_CBC_256_HMAC384_SIZE_BYTES);
    }
    ...
    fn test_bmk_restore_with_wrong_bk() {
        ...
        crypto_env.error = Some((CryptoFunc::AesCbc256Decrypt, CryptoError::AesDecryptFailed));
    ...
    fn test_ephemeral_key_gen_invalid_algorithm() {
        ...
        assert!(matches!(result, Err(CryptoError::InvalidAlgorithm)));
    }
    ...
    fn test_ephemeral_key_gen_crypto_env_random_error() {
        ...
        crypto_env.error = Some((CryptoFunc::GenerateRandom, CryptoError::InvalidParameter));
        let result = LMKeyDerive::ephemeral_key_gen(
            &crypto_env,
            MaskingKeyAlgorithm::AesCbc256Hmac384,
            output.len(),
    ...
    fn test_ephemeral_key_gen_crypto_env_kbkdf_error() {
        ...
        crypto_env.error = Some((CryptoFunc::KbkdfSha384, CryptoError::KbkdfError));
        let result = LMKeyDerive::ephemeral_key_gen(
            &crypto_env,
            MaskingKeyAlgorithm::AesCbc256Hmac384,
            output.len(),
    ...
    fn test_masked_bk3_gen_empty_bk3() {
        ...
        // Should fail with invalid parameter error
        assert!(result.is_err());
        assert!(matches!(result, Err(CryptoError::MaskedKeyPreEncodeFailed)));
    }
    ...
    fn test_masked_emphk_gen_crypto_env_kbkdf_error() {
        ...
        env.error = Some((CryptoFunc::KbkdfSha384, CryptoError::KbkdfError));
        let result = LMKeyDerive::masked_emphk_gen(
            &env,
            MaskingKeyAlgorithm::AesCbc256Hmac384,
            &TEST_EPHEMERAL_KEY,
